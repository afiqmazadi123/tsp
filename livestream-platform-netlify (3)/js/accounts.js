...
(function(window) {
  ...
  const DEFAULT_ACCOUNTS = [
    { id:'acc_afiq', name:'Afiq Mazadi', role:'Lead Live Operations', roleType:'admin', pin:'pbkdf2$120000$G4lfU3gfybNjn/XwpnLeBQ==$UkyRHxW1jsUSrJMJUL1kQdPdBUGyVPAfv/6JWcz8988=', avatarColor:'#0071e3', initials:'AM', canGrade:true, canEditWeights:true, canApprovePayroll:true, canManageRates:true, canManageAccounts:true, description:'Super Admin - Full system control, operations & grading' },
    { id:'acc_harto', name:'Ko Harto', role:'Head of Livestreaming', roleType:'executive', pin:'pbkdf2$120000$ZjBtBufzAs0PyieQzQSn3A==$74d2pMQyWlaty4d91qwTEL+c3CmMIkDIf/4wtkDSteQ=', avatarColor:'#bf5af2', initials:'KH', canGrade:true, canEditWeights:true, canApprovePayroll:true, canManageRates:true, canManageAccounts:true, description:'Executive Reviewer - Strategic management & senior grading' },
    { id:'acc_cici', name:'Cici', role:'Quality & Host Assessment Supervisor', roleType:'evaluator', pin:'pbkdf2$120000$KtpWRlnvIc8RffOSCf2UoQ==$5Nk0Di5MitFRNG6Rcmr20EXnrCqU/+PurlIpVbFdbC0=', avatarColor:'#ff2d55', initials:'CC', canGrade:true, canEditWeights:false, canApprovePayroll:false, canManageRates:false, canManageAccounts:false, description:'Lead Evaluator - Host QA, grooming & CTA assessment' },
    { id:'acc_aimee', name:'Aimee', role:'Livestream Operations Evaluator', roleType:'evaluator', pin:'pbkdf2$120000$eZiuLw0q5xttUyKZCA3UPw==$unfUZbKGtI2yi4NtRh9xiT6X4fSbX1fbogJJhGtEZ54=', avatarColor:'#30d158', initials:'AI', canGrade:true, canEditWeights:false, canApprovePayroll:false, canManageRates:false, canManageAccounts:false, description:'Shift Evaluator - Pinning & discipline assessment' },
    { id:'acc_finance', name:'Finance & HR Team', role:'Payroll Verification Specialist', roleType:'finance', pin:'pbkdf2$120000$PCsPMv/e/ORtBz2KzY9dBA==$CslUZUWZae2zkScZR+wrn/KMRgiW/5sJbr/Chlw9jqU=', avatarColor:'#ff9f0a', initials:'FN', canGrade:false, canEditWeights:false, canApprovePayroll:true, canManageRates:true, canManageAccounts:false, description:'Finance Verifier - Live hours validation & payments' },
    { id:'acc_brand', name:'Brand Partner Guest', role:'External Brand Partner', roleType:'brand', pin:'', avatarColor:'#64d2ff', initials:'BP', canGrade:false, canEditWeights:false, canApprovePayroll:false, canManageRates:false, canManageAccounts:false, description:'Brand Pitch Mode - Restricted operational access' }
  ];
  ...
  function sanitizeAccount(account) {
    ...
    const fallbackInitials = account.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'TM';
    account.initials = cleanText(account.initials || fallbackInitials, 'TM').slice(0, 2).toUpperCase();
    return account;
  }

  async function derivePin(pin, saltBytes, iterations = PBKDF2_ITERATIONS) {
    if (!window.crypto?.subtle) throw new Error('Secure PIN hashing is not supported by this browser.');
  ...
  const Accounts = {
    ...
    init() {
      const saved = typeof localStorage !== 'undefined' ? localStorage.getItem('fyc_sub_accounts') : null;
      if (saved) {
        try {
      ...
      this.accounts.forEach(acc => {
        sanitizeAccount(acc);
        if (acc.pin === undefined || acc.pin === null) { acc.pin = ''; modified = true; }
        if (acc.canManageRates === undefined) { acc.canManageRates = ['admin','executive','finance'].includes(acc.roleType); modified = true; }
        if (acc.canManageAccounts === undefined) { acc.canManageAccounts = ['admin','executive'].includes(acc.roleType); modified = true; }
        if (acc.canEditWeights === undefined) { acc.canEditWeights = ['admin','executive'].includes(acc.roleType); modified = true; }
      });

      this.currentAccountId = (typeof localStorage !== 'undefined' ? localStorage.getItem('fyc_current_account') : null) || 'acc_afiq';
      if (!this.getAccount(this.currentAccountId)) this.currentAccountId = this.accounts[0]?.id || 'acc_afiq';
    ...
    getAccounts() { return this.accounts || cloneDefaults(); },
    getReviewerAccounts() { return this.getAccounts().filter(a => a.canGrade); },
    getCurrentAccount() { return this.getAccount(this.currentAccountId) || this.accounts[0] || DEFAULT_ACCOUNTS[0]; },
    ...
    async setPin(accountId, pin) {
      ...
      this.persist();
      if (window.SupabaseEngine?.saveAccount) window.SupabaseEngine.saveAccount(acc);
      return acc;
    },
    ...
    can(permission) {
      return !!this.getCurrentAccount()?.[permission];
    },
    ...
    addAccount(newAcc) {
      ...
      const account = sanitizeAccount({
        ...source,
        id:'acc_' + Date.now(),
        initials:String(source.name || 'TM').split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase() || 'TM',
      ...
      if (String(rawPin || '').trim()) {
        this.setPin(account.id, rawPin).then(saveCloud).catch(console.warn);
      } else {
        saveCloud();
      }
      return account;
    ...
    updateAccount(id, updatedFields) {
      ...
      this.accounts[idx] = sanitizeAccount({
        ...existing,
        ...fields,
        initials:String(fields.name || existing.name).split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase()
      });
    ...
    deleteAccount(id) {
      if (this.accounts.length <= 1 || id === 'acc_afiq') return false;
